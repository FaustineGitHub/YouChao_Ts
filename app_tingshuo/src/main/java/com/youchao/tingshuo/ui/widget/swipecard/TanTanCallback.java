package com.youchao.tingshuo.ui.widget.swipecard;
...
/*        Log.d("TAG", "getSwipeThreshold() called with: viewHolder.itemView.getX() = [" + viewHolder.itemView.getX() + "]");
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (isLeftSwipe){
           //★实现循环的要点
            /*L.e("TAG","viewHolder.getLayoutPosition() = "+viewHolder.getLayoutPosition());
            Object remove = mDatas.remove(viewHolder.getLayoutPosition());
            mDatas.add(0, remove);
            Toast.makeText(mRv.getContext(), "左滑删除+", Toast.LENGTH_SHORT).show();
            mAdapter.notifyDataSetChanged();*/
        }else {
           /* Object remove = mDatas.add(viewHolder.getLayoutPosition());
            mDatas.remove(remove);*/
