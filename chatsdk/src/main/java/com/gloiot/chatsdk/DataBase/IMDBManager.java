    ...
    public synchronized void UpDateMessageInfo(String msgId, String msgIdFse, String receiveId, int messageState, String time, MessageChatCallBack messageChatCallBack) {
        ...
        while (cursor.moveToNext()) {
            if (messageState == 2) {  //失败
                ...
                Log.e("----", cursor.getInt(cursor.getColumnIndex("messageState")) + "   ----");
                if (cursor.getInt(cursor.getColumnIndex("messageState")) == 2) {
                    sqLiteDatabase.beginTransaction();
                    try {
                        long i = sqLiteDatabase.delete(tableName, "id=?", new String[]{cursor.getString(cursor.getColumnIndex("id"))});

                        if (i > 0) {
                            long j = sqLiteDatabase.insert(tableName, null, contentValues);

                            if (j > 0) {
                                sqLiteDatabase.setTransactionSuccessful();

                                updateConversationList(bean);
                ...
                } else if (cursor.getInt(cursor.getColumnIndex("messageState")) == 0) {
                    long i = sqLiteDatabase.update(tableName, contentValues, "id=?", new String[]{cursor.getString(cursor.getColumnIndex("id"))});
